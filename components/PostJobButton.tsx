...
export default function PostJobButton({ className, label = "Post a Job" }: Props) {
  ...
  async function handleClick() {
    ...
    if (user.user_metadata?.role !== "client") {
      showToastMsg("Only clients can post jobs. Please sign up or log in as a client.");
      return;
    }
    ...
    <>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-[200] flex items-start gap-3 bg-brand text-white px-5 py-4 rounded-2xl shadow-xl text-sm max-w-sm">
          ...
          <div>
            <p className="font-medium mb-0.5">Clients only</p>
      ...
      {showLogin && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-1">Log in to post a job</h2>
            <p className="text-sm text-gray-500 mb-6">
              Log in with your client account to continue
            </p>
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-4">
              ...
              >
                {loading ? "Logging in…" : "Log In & Post a Job"}
              </button>
            </form>
