...
export default function DashboardBottomNav({ role }: Props) {
  const pathname = usePathname();
  const navItems = role === "client" ? clientNav : contractorNav;

  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 bg-white border-t border-gray-200 flex lg:hidden">
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 transition-colors ${
              isActive ? "text-orange-500" : "text-gray-400 hover:text-gray-700"
            }`}
          >
            {item.icon}
